...
_repr_obj.maxlevel = (
    _repr_obj.maxtuple
) = _repr_obj.maxlist = _repr_obj.maxarray = (
    _repr_obj
).maxdict = _repr_obj.maxset = _repr_obj.maxfrozenset = _repr_obj.maxdeque = 3
...
def ensure_time(any_time: Union[str, int, datetime.time]) -> datetime.time:
    ...
    if isinstance(any_time, str):  # 使用regex处理字符串格式的时间
        if not (matched := pattern_for_str.match(any_time)):
            raise ValueError(
                f'Invalid time format for CSES format: {any_time!r}'
            )
        else:
        ...
        res = datetime.time(
            any_time // 3600, (any_time // 60) % 60, any_time % 60
        )
...
def serialize_time(
    dumper: yaml.representer.BaseRepresenter,  # ty: ignore [possibly-missing-submodule]
    any_time: datetime.time,
